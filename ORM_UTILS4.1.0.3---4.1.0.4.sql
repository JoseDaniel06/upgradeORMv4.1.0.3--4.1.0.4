...
AS
   /******************************************************************************
      ...
      4.1.0.3    16/06/2016      OPTIMA CONSULTING       Version 4.1.0.3 - Creación de job con el usuario dueño del package
                 04/10/2024      OPTIMA CORPORATION      Se carga scripts y README en repositorio Github oficial de ORM.   
   ******************************************************************************/
...
CREATE OR REPLACE PACKAGE BODY ORM.ORM_UTILS
AS
   /******************************************************************************
      NAME:       ORM_UTILS
      PURPOSE:
   ...
   ******************************************************************************/
   ...
   BEGIN
      BEGIN
         SELECT OPT_directorio.id_dir_padre
           INTO dir_padre
           FROM OPT_directorio
          WHERE OPT_directorio.id_directorio = pDirectorio;
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN
            dir_padre := NULL;
      ...
      IF dir_padre IS NOT NULL
      THEN
         Cadena :=
            DIRCONSREC (dir_padre) || '<' || TO_CHAR (pDirectorio) || '>';
      ELSE
         Cadena := '<' || TO_CHAR (pDirectorio) || '>';
      END IF;
   ...
   IS
      Cadena       VARCHAR2 (5000);
      dir_padre    NUMBER (9);
      dir_actual   VARCHAR2 (500);
   BEGIN
      BEGIN
         SELECT OPT_directorio.nombre, OPT_directorio.id_dir_padre
           INTO dir_actual, dir_padre
           FROM OPT_directorio
          WHERE OPT_directorio.id_directorio = pDirectorio;
      EXCEPTION
         WHEN NO_DATA_FOUND
      ...
      IF dir_padre IS NOT NULL
      THEN
         Cadena := SUBSTR (DIRPATH (dir_padre) || '\' || dir_actual, 1, 5000);
      ELSE
         Cadena := SUBSTR ('\' || dir_actual, 1, 5000);
      END IF;
   ...
   BEGIN
      ...
      IF dir_padre IS NOT NULL
      THEN
         Cadena := DIRPATHCOMAS (dir_padre) || ',' || TO_CHAR (pDirectorio);
      ELSE
         Cadena := TO_CHAR (pDirectorio);
      END IF;
